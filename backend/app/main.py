...
def expected_calendar(
    ...
    # If an override moved an occurrence from a different month into this one, the base
    # occurrence_date will not be generated by _expected_occurrences_in_range() for this month,
    # so we need to surface those moved overrides explicitly.
    expected_by_id: dict[int, ExpectedTransaction] = {t.id: t for t in expected_rows}
    for ovr in overrides:
        if ovr is None or ovr.moved_to_date is None:
            continue
        if ovr.cancelled:
            continue
        if not (start_date <= ovr.moved_to_date < end_date):
            continue
        # If the original occurrence is within this range, it is already handled by the
        # normal recurrence iteration above.
        if start_date <= ovr.occurrence_date < end_date:
            continue
        ...
        eff_account_id = ovr.account_id if ovr.account_id is not None else tx.account_id
        acc = accounts_by_id.get(eff_account_id)
        if acc is None:
            continue

        eff_kind = ovr.kind if ovr.kind is not None else tx.kind
        eff_amount = ovr.amount if ovr.amount is not None else tx.amount
        eff_description = ovr.description if ovr.description is not None else tx.description
        eff_reimbursable = (
            ovr.reimbursable
            if ovr.reimbursable is not None
            else bool(getattr(tx, "reimbursable", False))
        )
        eff_category_id = ovr.category_id if ovr.category_id is not None else tx.category_id
        cat = categories_by_id.get(eff_category_id) if eff_category_id is not None else None
        eff_variable = bool(getattr(tx, "variable", False))
        if getattr(ovr, "variable", None) is not None:
            eff_variable = bool(ovr.variable)
        eff_fg = ovr.fg_color if getattr(ovr, "fg_color", None) is not None else getattr(tx, "fg_color", None)
        eff_bg = ovr.bg_color if getattr(ovr, "bg_color", None) is not None else getattr(tx, "bg_color", None)

        items.append(
            ExpectedCalendarItemOut(
                expected_transaction_id=tx.id,
                date=ovr.moved_to_date,
